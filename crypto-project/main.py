...
def isEmail(txt): return True if re.match(r'[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+', txt) else False
...
def get_key():
    if os.path.exists(SECRET_KEY_FILE):
        ...
        return create_new_secret_key()

# Function to create or retrieve the secret key
...
# Function to encrypt the database with the current secret key
def encryptDatabase():
    with open(os.path.join(os.path.dirname(__file__), USER_PATH), 'r') as f:
        data = f.read()
    cipher = aes_encryption(str.encode(SECRET_KEY), str.encode(data))
...
# Function to decrypt the database with the current secret key
def decryptDatabase():
    with open(os.path.join(os.path.dirname(__file__), USER_PATH), 'r') as f:
        data = f.read()
...
while True:
        ...
        if os.path.exists(USER_PATH):
            ...
            if not salt == 0:        
                ...
                if match(USER_PATH,username, passwd,'LOGIN'):
                    ...
                    print('Password/Username Is Wrong')
                    input('Press Anything to Continue')
    ...
    if chc == '2':
        ...
        username = input('Input Username : ')
        while not isNotNumber(username,8,15):
            print('Username Invalid')
            print('Username Must Contain 8-15 Character with No Number')
            username = input('Input Username : ')    
        ...
        try:
            ...
            input('Press Anything to Continue')
    if chc == '0':
        encryptDatabase()  # Encrypt the databases before logging off
